...
export async function syncCatalogFromProvider(options: {
  ...
  if (!apply) {
    return {
      ...
      utilityGroups: utilityGroups.size,
...
function normalizeService(row: ProviderPriceListItem): NormalizedService {
  ...
  const category = inferCategory(gameName, row.category, row.type);
  return {
    gameName,
    gameSlug,
    category,
...
function isTopupPulsaService(row: NormalizedService) {
  if (!isPulsaService(row.gameName, row.productName)) return false;
  if (!/pulsa/i.test(row.sourceCategory)) return false;
...
function isKnownGameService(row: NormalizedService) {
  ...
  if (/malaysia|singapore|philippines|region|trial|test produk/i.test(text)) {
    return false;
  }
  return true;
}
...
function normalizeProductName(value: string, gameName: string) {
  const normalized = normalizeName(value)
    .replace(/^Mobilelegend\s*-\s*/i, "Mobile Legends ")
    .replace(/^Mobile Legends\s*-\s*/i, "Mobile Legends ")
    .replace(/^Go Pay\s+/i, "GoPay ")
    .replace(/^Shopee Pay\s+/i, "ShopeePay ")
    .replace(/^Cek Nama Pengguna\s+/i, "Cek Nama ");

  if (gameName === "Mobile Legends") {
    return normalized.replace(/^Mobilelegend/i, "Mobile Legends");
  }
  return normalized;
}
...
function normalizeGameName(brand: string, productName: string, category: string) {
  ...
  if (isWalletService(normalizedBrand, normalizedProduct)) {
    if (/\bdana\b/i.test(text)) return "Saldo DANA";
    if (/go\s*pay|gopay/i.test(text)) return "Saldo GoPay";
    if (/\bovo\b/i.test(text)) return "Saldo OVO";
